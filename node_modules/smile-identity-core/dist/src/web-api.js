...
const id_api_1 = require("./id-api");
const helpers_1 = require("./helpers");
const web_token_1 = require("./web-token");
    ...
    if (!('entered' in idInfo) || entered === 'false') {
        if (jobType === 6) {
            ...
            if (!idInfo.country || idInfo?.country?.length === 0) {
                throw new Error('Please make sure that country is included in the id_info');
            }
        }
        else if (jobType === 11) {
            ...
            ['country', 'id_type'].forEach((key) => {
                const idKey = key;
                if (!idInfo[idKey] || idInfo[idKey].length === 0) {
...
const validateBooleans = (options) => {
    ...
    booleanKeys.forEach((key) => {
        const optionKey = key;
        obj[key] = checkBoolean(key, options[optionKey]);
    });
    return obj;
};
/**
...
const validateImages = (images, useEnrolledImage, jobType) => {
    ...
    if ((!images.some(hasSelfieImage) || images.length === 0) &&
...
const configureImagePayload = (images) => images.map(({ image, image_type_id }) => {
    ...
    if ([0, 1].includes(image_type_id)) {
        return {
            ...
            image: '',
            file_name: path_1.default.basename(image),
        };
    }
    return {
        image_type_id: imageTypeId,
 ...
 */
const configurePrepUploadPayload = ({ api_key, callback_url, idInfo, partner_id, partner_params, timestamp, use_enrolled_image, }) => ({
    callback_url,
    ...
    use_enrolled_image,
    ...idInfo,
    ...new signature_1.default(partner_id, api_key).generate_signature(timestamp),
 ...
 */
const queryJobStatus = ({ api_key, partner_id, partner_params, url: sidUrl, return_history, return_images, }, counter = 0) => new Promise((resolve, reject) => {
    // call job status for the result of the job
    const timeout = counter < 4 ? 2000 : 4000;
    const updatedCounter = counter + 1;
    const retryFunc = (currentCounter) => {
        setTimeout(() => {
            queryJobStatus({
                api_key,
                partner_id,
    ...
    new utilities_1.Utilities(partner_id, api_key, sidUrl)
        .get_job_status(partner_params.user_id, partner_params.job_id, {
        return_history,
        return_images,
    })
        .then((body) => {
 ...
 * @throws {Error} - if the request fails or times out.
 */
const uploadFile = (data, zipFile, signedUrl, smile_job_id) => axios_1.default
    .put(signedUrl, zipFile, {
    headers: {
...
const zipUpFile = (images, infoJson) => {
    ...
    images
        ...
        .forEach((image) => {
        zip.file(path_1.default.basename(image.image), fs_1.default.readFileSync(image.image));
    });
 ...
 */
const setupRequests = (payload) => axios_1.default
    .post(`https://${payload.url}/upload`, configurePrepUploadPayload(payload))
    .then(({ data }) => Promise.all([
    zipUpFile(payload.images, configureInfoJson(payload, data)),
    Promise.resolve(data),
]))
    .then(([zipFile, { upload_url, smile_job_id }]) => uploadFile(payload, zipFile, upload_url, smile_job_id));
class WebApi {
    ...
    get_job_status(partner_params, options) {
        return new utilities_1.Utilities(this.partner_id, this.api_key, this.url).get_job_status(partner_params.user_id, partner_params.job_id, options);
    }
    /**
    ...
    get_web_token(requestParams) {
        return (0, web_token_1.getWebToken)(this.partner_id, this.api_key, this.url, requestParams, this.default_callback);
    }
    /**
    ...
    submit_job(partner_params, image_details, id_info, options = {}) {
        try {
            ...
            const useIdApi = jobType === constants_1.JOB_TYPE.BASIC_KYC ||
                jobType === constants_1.JOB_TYPE.BUSINESS_VERIFICATION;
            if (useIdApi) {
                return new id_api_1.IDApi(this.partner_id, this.api_key, this.url).submit_job(partner_params, id_info);
            }
